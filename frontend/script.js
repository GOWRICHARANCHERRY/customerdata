...
function checkAuthentication() {
  ...
  if (currentUser.screenshotRestricted) {
    ...
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:#000;color:#fff;z-index:999999;display:flex;justify-content:center;align-items:center;flex-direction:column;font-size:24px;text-align:center;opacity:0;pointer-events:none;transition:opacity 0s;';
    ...
    function block() { overlay.style.opacity = '1'; overlay.style.pointerEvents = 'auto'; }
    function unblock() { overlay.style.opacity = '0'; overlay.style.pointerEvents = 'none'; }
    ...
    document.addEventListener('touchcancel', block);
    document.addEventListener('touchstart', () => { if (document.hidden) block(); });
    ...
    setInterval(() => {
      if (document.hidden !== wasHidden) { wasHidden = document.hidden; if (document.hidden) block(); else unblock(); }
      if (document.hasFocus() !== wasFocused) { wasFocused = document.hasFocus(); if (!wasFocused) block(); else unblock(); }
    }, 50);
  }
}
