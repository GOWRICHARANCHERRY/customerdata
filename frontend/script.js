...
function checkAuthentication() {
  ...
  if (currentUser.screenshotRestricted) {
    ...
    // Wrap all content in a hide-able container
    let contentWrap = document.getElementById('screenshotContent');
    if (!contentWrap) {
      contentWrap = document.createElement('div');
      contentWrap.id = 'screenshotContent';
      while (document.body.firstChild) contentWrap.appendChild(document.body.firstChild);
      document.body.appendChild(contentWrap);
    }

    // Full-screen overlay toggles opacity (faster than display none/flex)
    const overlay = document.createElement('div');
    overlay.id = 'screenshotOverlay';
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:#000;color:#fff;z-index:999999;display:flex;justify-content:center;align-items:center;flex-direction:column;font-size:24px;text-align:center;opacity:0;pointer-events:none;will-change:opacity;';
    overlay.innerHTML = '<div style="font-size:80px;margin-bottom:20px;">🚫</div><div>Content Protected</div><div style="font-size:14px;margin-top:10px;color:#999;">Screenshot not allowed</div>';
    ...
    // Fullscreen — Android often exits fullscreen during screenshot button combo
    try { document.documentElement.requestFullscreen(); } catch(e) {}
    document.addEventListener('fullscreenchange', () => { if (!document.fullscreenElement) block(); });

    // All visibility/focus events
    document.addEventListener('visibilitychange', () => { if (document.hidden) block(); else unblock(); });
    window.addEventListener('blur', block);
    window.addEventListener('focus', unblock);
    window.addEventListener('pageshow', (e) => { if (e.persisted) block(); });

    // Touch events — screenshot button combo often triggers touchcancel or pointercancel
    document.addEventListener('touchcancel', block, true);
    document.addEventListener('pointercancel', block, true);
    document.addEventListener('touchstart', () => { if (document.hidden) block(); }, true);

    // Print / screenshot key
    document.addEventListener('contextmenu', e => e.preventDefault());
    document.addEventListener('copy', e => e.preventDefault());
    document.addEventListener('cut', e => e.preventDefault());
    document.addEventListener('keydown', e => { if (e.key === 'PrintScreen') { e.preventDefault(); block(); } });
    window.addEventListener('beforeprint', block);
    window.addEventListener('afterprint', unblock);

    // Online/offline — some screenshot tools trigger offline state
    window.addEventListener('offline', block);

    // requestAnimationFrame loop — fires BEFORE every composite (fastest JS path)
    let wasHidden = !document.hidden;
    let wasFocused = document.hasFocus();
    function rafCheck() {
      const h = document.hidden;
      const f = document.hasFocus();
      if (h !== wasHidden) { wasHidden = h; if (h) block(); else unblock(); }
      if (f !== wasFocused) { wasFocused = f; if (!f) block(); else unblock(); }
      requestAnimationFrame(rafCheck);
    }
    requestAnimationFrame(rafCheck);

    // 50ms interval as backup in case rAF stalls
    setInterval(() => {
      const h = document.hidden;
      const f = document.hasFocus();
      if (h !== wasHidden) { wasHidden = h; if (h) block(); else unblock(); }
